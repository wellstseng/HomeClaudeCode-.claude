...
| `Stop` | 對話結束前 | 閘門：未同步則阻止結束 + Fix Escalation 信號注入（retry≥2 自動觸發升級） |
...
| `rules/session-management.md` | 對話管理 + 續航 + 自我迭代 + 精確修正升級（V2.12） |
...
使用者訊息 → UserPromptSubmit hook (workflow-guardian.py)
  ...
  ├─ Fix Escalation: retry_count≥2 → 注入 [FixEscalation] 信號 (V2.12)
...
- **V2.12 新增 Fix Escalation Protocol**：同一問題修正第 2 次起強制 6 Agent 精確修正會議，Guardian 自動偵測 + /fix-escalation skill
