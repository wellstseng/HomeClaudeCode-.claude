...
│
├── hooks/
│   └── workflow-guardian.py          ← 統一 Hook 入口 (~1400 行)
│
├── memory/                           ← 全域記憶層
...
### Vanilla Claude Code vs + Atomic Memory V2.3

| 指標 | Vanilla Claude Code | + Atomic Memory V2.3 |
|------|--------------------|-----------------------|
...
```
  使用者     Guardian Hook    Claude Code    Vector Svc    Ollama     檔案系統
    ...
    │            │                │              │           │           │
    │  ┌──── SessionEnd Hook ───────────────────────────────────────────┐
    │  │ [Q] 自動生成 episodic atom (session 摘要, TTL 24d)  │           │
    │  │ [R] 觸發 incremental index (若有 atom 變動) ──────>│           │
    │  │ [S] 寫入 state phase="done"             │           │           │
    │  │                                         │           │           │
...
| **本地 LLM** | Ollama qwen3 | 語意處理：embedding 生成、query rewrite、re-ranking、intent 分類 | ~200-500ms |
