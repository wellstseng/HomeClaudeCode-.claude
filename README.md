# 原子記憶系統 Atomic Memory V2.8

> **Claude Code 跨 Session 知識管理引擎**
> Hybrid RECALL | Dual LLM | Workflow Guardian | Self-Iteration Engine | Wisdom Engine
...
| 6 | **自我迭代** | 系統自身也隨使用演進，8 條原則有跨學科理論背書 |
...
│
├── hooks/
│   ├── workflow-guardian.py          ← 統一 Hook 入口 (~2100 行, 含 V2.7 Self-Iteration + Quality Feedback)
│   └── wisdom_engine.py             ← V2.8 Wisdom Engine (~250 行, 因果圖+情境分類+反思引擎)
│
├── memory/                           ← 全域記憶層
...
│   ├── decisions.md                 ← [固] 全域決策
│   ├── hardware.md                  ← [觀] 硬體配置
│   ├── failures.md                  ← [觀] 失敗模式記憶 (V2.7)
│   ├── toolchain.md                 ← [觀] 工具鏈實戰記憶 (V2.7)
│   ├── openclaw-self-iteration.md   ← [固] 自我迭代理論背書 (V2.6)
│   ├── SPEC_*.md                    ← 系統規格 (參考用)
│   ├── wisdom/                      ← V2.8 Wisdom Engine 資料
│   │   ├── causal_graph.json        ← 因果圖 (有向加權圖 + BFS)
│   │   ├── reflection_metrics.json  ← 反思統計 (滑動窗口)
│   │   └── DESIGN.md                ← 設計文件
...
├── workflow/
│   ├── config.json                  ← 統一設定檔
│   ├── state-{session-id}.json      ← Session 狀態追蹤 (ephemeral, schema 1.2)
│   └── last_review_marker.json      ← V2.6 定期檢閱計數器
│
...
└── commands/                         ← Slash commands (/skills)
    ...
    ├── resume.md                    ← /resume 自動續接 Session (MCP 桌面自動化)
    └── talk-to-openclaw.md          ← /talk-to-openclaw WebSocket 通訊
...
### Vanilla Claude Code vs + Atomic Memory V2.8

| 指標 | Vanilla Claude Code | + Atomic Memory V2.8 |
|------|--------------------|-----------------------|
| **Session 啟動延遲** | ~0ms | +200-850ms (Guardian init + Vector Service check + review check + Wisdom reflection) |
| **每次 Prompt 額外延遲** | ~0ms | +300-600ms (keyword ~10ms + vector ~200-500ms + intent ~1ms + wisdom ~5ms) |
...
| **SessionEnd 延遲** | ~0ms | +100-5000ms (transcript extraction + cross-session + V2.6 metrics + oscillation) |
| **CLAUDE.md Token** | 0 | ~1,400-1,900 tokens (V2.7 精簡後 144 行，每 session 固定) |
| **MEMORY.md Token** | 0 | ~50-80 tokens (索引表 ≤30 行，每 session 固定) |
| **Atom 注入 Token** | 0 | ~200-1,500 tokens/次 (按需，命中才載入) |
| **典型 Session 總 Overhead** | 0 | **~1,600-3,500 tokens (~0.8-1.8% of 200K context)** |
...
sequenceDiagram
    ...
    rect rgba(100, 150, 255, 0.1)
        ...
        G->>G: [4] V2.6 定期檢閱到期? + 成熟度階段計算
        G->>C: [5] 注入: atom 索引清單 + Guardian 狀態 + 檢閱提醒(若到期)
    end
    ...
    rect rgba(100, 200, 100, 0.1)
        ...
        note over G,O: V2.4 — 回應知識捕獲 (非同步)<br/>V2.5 — Keyword Boost<br/>V2.6 — Self-Iteration metrics
    ...
    rect rgba(150, 100, 255, 0.1)
        ...
        G->>G: [S] V2.6 收集 iteration_metrics (atoms referenced/modified)
        G->>G: [T] V2.6 震盪偵測 (近 3 session 同 atom 改 2+ 次?)
        G->>G: [U] 自動生成 episodic atom (含 修改 atoms 紀錄)
        G->>V: [V] incremental index (若有 atom 變動)
        G->>G: [W] 寫入 state phase="done"
    end

    note over U,F: ✅ 知識已保存到 atoms + vector DB<br/>✅ Hybrid Search + Keyword Boost (V2.5)<br/>✅ 回應知識自動萃取 (V2.4)<br/>✅ 跨 session 重複知識自動晉升 (V2.4)<br/>✅ Self-Iteration 指標追蹤 + 震盪偵測 (V2.6)<br/>✅ Wisdom Engine 因果警告 + 情境分類 + 反思 (V2.8)<br/>✅ 下次 session 自動載入
```
...
flowchart TD
    ...
    P --> VS["Vector Semantic Search<br/><i>ChromaDB + qwen3-embedding:0.6b ~200-500ms</i>"]

    KW --> MG["Ranked Merge"]
    INT --> MG
    VS --> KB["V2.5 Keyword Boost<br/><i>大寫詞/引號短語/中文專有名詞<br/>雙命中 +0.1 | 救回 +0.05</i>"]
    KB --> MG

    MG --> SC["FinalScore = 0.45×Semantic + 0.15×Recency<br/>+ 0.20×IntentBoost + 0.10×Confidence<br/>+ 0.10×(Confirmations + TypeBonus)<br/>+ KeywordBoost"]
...
| **Hybrid Search (V2.5)** | 向量結果疊加 keyword boost：大寫詞/引號短語/中文專有名詞，提升專有名詞召回率 |
| **Self-healing Cache (V2.5)** | ChromaDB collection 快取 + 失效自動 invalidate/retry + 連續失敗警告升級 |
...
| **Self-Iteration Engine (V2.6)** | 8 條演進原則自動化：品質函數、震盪偵測、成熟度模型、定期檢閱觸發。理論背書見 `openclaw-self-iteration.md` |
| **Iteration Metrics (V2.6)** | SessionEnd 自動收集 atoms_referenced + atoms_modified，episodic atom 記錄修改歷史供震盪偵測 |
| **Periodic Review (V2.6)** | SessionStart 檢查距上次檢閱的 session 數（預設每 6 session），到期時注入提醒 |
| **Failures / Toolchain Atoms (V2.7)** | 失敗模式記憶（環境踩坑/假設錯誤/模式誤用/品質回饋）+ 工具鏈實戰記憶（已驗證指令/路徑/特殊配置） |
| **Output Quality Feedback (V2.7)** | PostToolUse 偵測跨 session 重寫（>30% 變動），結果寫入 episodic + failures.md |
| **Wisdom Engine: Causal Graph (V2.8)** | 有向加權因果圖 + BFS depth=2，修改檔案時自動警告關聯影響（≤3 warnings, ~90 tokens） |
| **Wisdom Engine: Situation Classifier (V2.8)** | 加權評分函數分類任務複雜度 → direct/confirm/plan，小任務零注入不打擾 |
| **Wisdom Engine: Reflection Engine (V2.8)** | 滑動窗口統計 first_approach_accuracy + over_engineering_rate + 盲點偵測（<70% 標記） |
| **/resume Skill** | 自動續接 Session：收集狀態→生成 prompt→MCP 桌面自動化開新 VS Code 視窗→貼上→執行 |
...
### 6. Self-Iteration Engine（V2.6）

系統自身也隨使用演進。8 條原則各有跨學科理論背書（心理學、數學、哲學、工程、AI）：

| # | 原則 | 自動化 | 說明 |
|---|------|--------|------|
| 1 | 品質函數 | Claude 行為 | 確認(+)/糾正(−)/無回饋(0) 三類訊號驅動 |
| 2 | 收斂優先 | Claude 行為 | 新增前先檢查可合併的既有規則 |
| 3 | 證據門檻 | Hook 自動 | ≥2 獨立 session 觀察才建立規則 |
| 4 | 淘汰勇氣 | Claude 行為 | 每新增 1 條 → 檢查淘汰 1 條 |
| 5 | 震盪偵測 | Hook 自動 | 3 session 內同 atom 改 2+ 次 → 暫停 |
| 6 | 成熟度模型 | Hook 自動 | 學習期(<15) / 穩定期(15-50) / 成熟期(>50) |
| 7 | 三維演進 | Claude 行為 | 精確度 / 協助力 / 良善性 Pareto 平衡 |
| 8 | 演進邊界 | Claude 行為 | 只改行動/知識，禁止自改 Confidence |

```mermaid
flowchart TD
    SE["SessionEnd"] --> M["_collect_iteration_metrics()<br/><i>atoms_referenced + atoms_modified</i>"]
    M --> O["_detect_oscillation()<br/><i>掃描近 3 session episodic<br/>同 atom 修改 2+ 次?</i>"]
    O -- "震盪" --> WARN["⚠ 標記暫停 + stderr 警告"]
    O -- "穩定" --> EP["生成 episodic atom<br/><i>含「修改 atoms:」紀錄</i>"]

    SS["SessionStart"] --> MAT["_calculate_maturity_phase()<br/><i>learning / stable / mature</i>"]
    MAT --> REV["_check_periodic_review_due()<br/><i>距上次 ≥ 6 sessions?</i>"]
    REV -- "到期" --> INJ["注入檢閱提醒到 additionalContext"]
    REV -- "未到期" --> SKIP["跳過"]
```
...
| V2.5 | 2026-03-06 | **Hybrid Search Keyword Boost + Self-healing Cache**：向量結果疊加 keyword matching（專有名詞召回率提升）、ChromaDB collection 快取 + 失效自動恢復 |
| V2.6 | 2026-03-10 | **Self-Iteration Engine**：8 條演進原則（跨學科理論背書）+ 品質函數 + 震盪偵測 + 成熟度模型（學習/穩定/成熟期）+ 定期檢閱自動觸發 + iteration_metrics state 追蹤 |
| V2.7 | 2026-03-10 | **CLAUDE.md 精簡 50%**（289→144 行，移除 hook 實作細節與重複偏好）+ **failures.md / toolchain.md** atoms 新增 + **Output Quality Feedback**（PostToolUse 跨 session 重寫偵測）+ 跨專案模式掃描 |
| **V2.8** | **2026-03-11** | **Wisdom Engine**：因果圖（有向加權 BFS + Bayesian confidence）+ 情境分類器（加權評分 direct/confirm/plan）+ 反思引擎（滑動窗口盲點偵測）+ `/resume` skill（MCP 桌面自動化跨 session 續接） |
